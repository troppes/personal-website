...
export async function load() {
	...
	try {
		...
		photos = photosResponse.data.map(
			(
				/** @type {{ id: { toString: () => any; }; description: any; tags: { gallery_tags_id: any; }[]; alt: any; photo: string; }} */ post
			) => ({
				id: post.id.toString(),
	...
	try {
		...
		tags = tagsResponse.data
			.map((/** @type {{ id: { toString: () => any; }; tag: any; gallery: {}; }} */ tag) => ({
				id: tag.id.toString(),
				tag: tag.tag,
				timesUsed: Object.keys(tag.gallery).length
			}))
			.sort(
				(/** @type {{ timesUsed: number; }} */ a, /** @type {{ timesUsed: number; }} */ b) =>
